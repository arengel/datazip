...
try:
    ...
    def _encode_ndarray(self: DataZip, name: str, data: np.ndarray, **kwargs) -> dict:
        ...
        return {
            "__type__": "ndarray",
            "__loc__": self.encode_loc_helper(f"{name}.npy", data, temp.getvalue()),
        }

    def _decode_ndarray(self: DataZip, obj) -> np.ndarray:
        return np.load(BytesIO(self.read(obj["__loc__"])))
...
try:
    ...
    def _encode_pd_df(self: DataZip, name: str, df: pd.DataFrame, **kwargs) -> dict:
        ...
        try:
            return {
                ...
                "__loc__": self.encode_loc_helper(
    ...
    def _encode_pd_series(self: DataZip, name: str, df: pd.Series, **kwargs) -> dict:
        if self._ids_for_dedup and (loc := self._ids.get((id(df), type(df)), None)):
    ...
    def _decode_pd_df(self: DataZip, obj: dict) -> pd.DataFrame:
        return pd.read_parquet(BytesIO(self.read(obj["__loc__"])))

    def _decode_pd_series(self: DataZip, obj: dict) -> pd.Series:
        out: pd.Series = pd.read_parquet(BytesIO(self.read(obj["__loc__"]))).squeeze()
...
try:
    ...
    def _encode_pl_df(self: DataZip, name: str, df: pl.DataFrame, **kwargs) -> dict:
        ...
        return {
            ...
            "__loc__": self.encode_loc_helper(f"{name}.parquet", df, temp.getvalue()),
        }

    def _encode_pl_ldf(self: DataZip, name: str, df: pl.LazyFrame, **kwargs) -> dict:
        """Write a polars df in the ZIP as parquet."""
        if self._ids_for_dedup and (loc := self._ids.get((id(df), type(df)), None)):
    ...
    def _encode_pl_series(self: DataZip, name: str, df: pl.Series, **kwargs) -> dict:
    ...
    def _decode_pl_df(self: DataZip, obj: dict) -> pl.DataFrame:
        return pl.read_parquet(BytesIO(self.read(obj["__loc__"])), use_pyarrow=True)

    def _decode_pl_ldf(self: DataZip, obj) -> pl.LazyFrame:
        return pl.read_parquet(
            BytesIO(self.read(obj["__loc__"])), use_pyarrow=True
        ).lazy()

    def _decode_pl_series(self: DataZip, obj: dict) -> pl.Series:
        return (
...
try:
    ...
    def _encode_plotly(self: DataZip, name: str, item) -> dict:
        return {
            "__type__": "pgoFigure",
            "__loc__": self.encode_loc_helper(f"{name}.pkl", item, pickle.dumps(item)),
        }

    def _decode_plotly(self: DataZip, obj: dict) -> go.Figure:
        return pickle.load(BytesIO(self.read(obj["__loc__"])))  # noqa: S301
